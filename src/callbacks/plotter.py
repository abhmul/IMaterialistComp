...
class Plotter(Callback):
    ...
    :param monitor: The metric to monitor and plot. Set to "loss" to plot the
        loss. Defaults to plotting the loss.
    :param scale: The scale to plot with. Options are "linear" and "log".
        Defaults to linear.
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if train_score is None:
            warnings.warn(
                "Monitoring metric {monitor} cannot be found for train".format(
        ...
        if val_score is None:
            warnings.warn(
                "No validation metric {monitor} can be found, cannot plot"
                "validation scores".
