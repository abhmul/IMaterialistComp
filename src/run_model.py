...
parser.add_argument("--test", action="store_true", help="Whether or not to test the model.")
parser.add_argument("--plot", action="store_true", help="Whether or not to plot during training.")
parser.add_argument("--load_model", action="store_true", help="Whether or not to continue training a saved model")


def train_model(model: Model, train_dataset: ImageDataset, val_dataset: ImageDataset, augmenters=tuple(),
                epochs=100, batch_size=32, plot=False, load_model=False, **kwargs):
    ...
    callbacks = [ModelCheckpoint(utils.get_model_path(model.run_id), monitor="val_loss",
                                 save_best_only=True, save_weights_only=True),
                 ModelCheckpoint(utils.get_model_path(model.run_id + "_acc"), monitor="val_acc",
                                 save_best_only=True, save_weights_only=True),
                 Plotter(monitor="loss", scale="log", plot_during_train=plot,
                         save_to_file=utils.get_plot_path(model.run_id), block_on_end=False),
                 Plotter(monitor="acc", scale="linear", plot_during_train=plot,
                         save_to_file=utils.get_plot_path(model.run_id + "_acc"), block_on_end=False)]

    # Train the model
    history = model.fit_generator(traingen, steps_per_epoch=5, epochs=epochs, verbose=1,
                                  callbacks=callbacks, validation_data=valgen, validation_steps=5)
...
def test_model(model: Model, test_dataset: ImageDataset, augmenters=tuple(), batch_size=32, **kwargs):
...
def test(data: utils.IMaterialistData, run_config, model=None):
    ...
    data.save_submission(utils.get_submission_path(model.run_id), predictions, test_data.ids, thresholds=run_config["threshold"])


if __name__ == "__main__":
    args = parser.parse_args()
    current_run_config = load_config(args.run_id)
    imaterialist_data = utils.IMaterialistData(path_to_train=current_run_config["path_to_train"],
